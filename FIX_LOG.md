...
- 把功能缺口里的 **F09（训练权重回填编辑器）** 做完：生成的脚本能导出训练结果，编辑器能安全地
  把它灌回这张图——连线粗细 / 颜色、热力图、右栏数值全跟着变，然后还能接着改、接着重新编译。
- 安全的前提是**结构指纹**：一个只由**结构**算出来的 sha256。权重、阈值、偏置、冻结标记
  **一律不进指纹**（它们正是要回填的东西，进去了就永远对不上）。指纹对不上、连接数对不上、
  形状对不上，一律明确拒绝，**绝不半填**——填错的权重是静默的，跑出来全是错的，比报错难查得多。
- 顺手修好一条**本来就在挂的回归**：`tools/verify_gpt6_integration.mjs`。第 77 轮给
  `aiSessSnap` 加了「同状态不许盖新时间」的指纹槽（`aiSessSig` / `AI_SNAP`），但那支回归是按函数名
  一个个抠源码的，两个新符号没进它的清单，于是它从第 77 轮起一直 `ReferenceError`。补上清单和
  一个同形的 `AI_SNAP` 之后恢复绿色（**这是测试自己的漏洞，不是产品缺陷**）。

### 2. 逐项改动

**结构指纹（`main.js` F09 段，约 8690 行起）**

- 纯 JS 的 SHA-256（`sha256New`）+ 自己编的 UTF-8（生成的产物沙箱里不保证有 `TextEncoder`）。
- `topoCanonText(m)` 是规范文本：规模、输入 / 输出神经元、每个权重块的 `k/n/sg/rep` 与行列 id、
  每个算子节点的 `op/name/outShape/ins/land/params(名字:类型:形状:角色:共享)`、以及激活码。
  **数值一个不写**。连接的两端另外按 int32 小端分块（1<<20 条一块）喂进去——1680 万条边也不用
  为它再开一份大缓冲。
- `irTopologyId(m)` / `graphTopologyId()`：同一张图训练前训练后算出来**一样**。
- 为什么不用编译下标当参数 ID：连接在编译时是**按目标波次重排过**的，换个排序就全错位；
  而 (源, 目标) 这对神经元编号是稳定的。**指纹认「同一张图」，(源, 目标) 认「同一条边」。**

**权重文档（`weightsDocOf` / `weightsReadDoc` / `weightsCheck` / `weightsApply`）**

- 文档分两半：`layout` 是稳定 ID（每条连接的两端、每个块、每个算子参数），`values` 是数值
  （每条连接的权重、偏置、块的矩阵、算子参数）。`meta` 只写"这是什么、什么时候导的"。
- `weightsCheck` 是**先全查完再写**：指纹、连接数、逐条两端、偏置个数、块个数与元素数、
  算子参数形状全部过一遍，有一处不对就抛一句人话，一个字节都不改。
- `weightsApply` 整件事是**一个撤销格**（`snapshot()` 一格），填错了 Ctrl+Z 一把全回来。
  冻结的连接 / 神经元默认跳过并在返回值里报出 `locked`，`{force:true}` 才连冻结的一起覆盖。
  写完把块的 `texTag` 与算子的 `texTag` 清掉，热力图按新权重重画。
- 阈值不进模型（这条老规矩没变），所以**阈值不在回填范围内**——回填的是权重、偏置、块、算子参数。

**生成侧（`generatePyTorch`）**

- 编译时算一次指纹，写成 `NF_TOPOLOGY = '...'`；另外把 `NF_GRAPH` / `NF_LAYOUT_BLOCKS` /
  `NF_LAYOUT_PARAMS` 三张表当字面量带进产物（都只跟结构有关，规模极小）。
- 新增 `export_weights(path)` 与命令行 `--export-weights PATH`：把 `weight / bias / bw /
  算子参数` 连同 `NF_TOPOLOGY` 写成一份 JSON。块只导共享组的**代表块**（`rep`），
  算子参数按 `o<i>_p<k>` 回读——两边说的是同一件事。
- **NaN / Inf 在导出这一步就拒绝**：`json.dump` 会把它们写成 `NaN` / `Infinity`，那不是合法
  JSON，编辑器 `JSON.parse` 炸出来的是一句"语法错"，用户根本想不到是训练发散了。
- 循环网不用额外处理：IR 里 `src / dst / w / eids` 本来就是**前向 + 回边同一个数组**，
  回边权重就在 `weight` 的尾部，回填自动覆盖到。

**界面**

- 编译菜单加「回填训练权重...」（自己挑文件），训练页加同一个入口 + 「算一下当前图的结构指纹」
  按钮（大工程要过一遍整张图，所以按一帧后再算，不让面板卡住）。

**AI 侧**

- 新增两个工具：`weights_doc`（拿当前图的指纹与 layout）、`weights_import`（把 JSON 灌回来，
  可带 `force`）。脚本接口补 `NF.weightsDoc / weightsApplyText / weightsApplyBuffer / weightsLoadFile`，
  角色表相应补上（`weightsDoc` 只读、`weightsApplyText` 可撤销、`weightsLoadFile` 碰文件）。
- 手册升到 **r60**，加【回填训练权重（把训练结果灌回图里）】一节（含指纹规则、冻结语义、
  NaN 那条、以及四个脚本接口）。菜单命令一览也跟着补了 `weights-import`。

### 3. 验收

- 新增 `tools/verify_weights_roundtrip.py`：**75 条**。用生产生成器现造五个夹具
  （前馈 / 循环 / 有状态神经元 / 权重块（走 model.bin）/ 算子节点），每个走一遍完整往返：
  导出的那一份与**编辑器侧现算的那一份**对账（指纹一字不差、每条边序相同、块表与算子参数表一致、
  数值按 float32 逐位相同）；把可训练张量清零再按文档填回去，前向输出**逐位相同**；
  权重 / 偏置 / 块改了**指纹不变**而数值确实变了（"数值不进指纹"这条设计断言）；
  NaN 被拒绝且不留半个文件；命令行 `--export-weights` 与直接调用内容一致。
  五个夹具的指纹两两不同。
- 自测页新增第 46 组（**16 条**）：指纹是 64 位十六进制；改权重 / 阈值 / 偏置 / 冻结指纹不变；
  加一条连接、加一个神经元指纹必变；按 (源, 目标) 定位到边并回填；回填是一个撤销格；
  冻结默认跳过、`force` 才覆盖；指纹 / 非 JSON / 连接数三种坏文件都被明确拒绝；
  `weights_doc` / `weights_import` 两个工具真的能用。
- 全量回归：**17 个校验页全过**（自测页 **1010 PASS / 0 FAIL**，比上轮 +16）；
  **16 个 Python 回归 + 3 个 mjs 回归全过**（`verify_weights_roundtrip` 75、`verify_import` 283、
  `verify_train_manage` 42、`verify_train_ce` 37、`verify_audit_codegen` 39、`verify_cross` 117…）。
